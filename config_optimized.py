"""
High-performance configuration for 32-core GPU machines.
...
class OptimizedStonyBrookConfig:
    ...
    abbreviations = [
        "SBU", "SUNY SB", "SUSB", "Stony Brook", 
        "State University of New York at Stony Brook",
        "SUNY Stony Brook", "University at Stony Brook"
    ]
...
class OptimizedPipelineConfig:
    """Pipeline configuration optimized for 32-core processing"""
    
    # Parallel processing
    max_workers: int = 32           # Full CPU utilization
...
class OptimizedConfig:
    ...
    def _load_from_env(self):
        ...
        # PDF configuration
        self.pdf.download_enabled = os.getenv("ENABLE_PDF_DOWNLOAD", "false").lower() == "true"
        ...
        self.pipeline.output_directory = os.getenv("OUTPUT_DIR", self.pipeline.output_directory)
        self.export.compression_enabled = os.getenv("ENABLE_COMPRESSION", "true").lower() == "true"
    ...
    def _check_performance_settings(self):
        ...
        if self.pipeline.max_workers > available_cores:
            print(f"WARNING: max_workers ({self.pipeline.max_workers}) > available cores ({available_cores})")
            
        if self.pdf.download_enabled and self.pipeline.default_num_authors > 1000:
            print("WARNING: PDF downloads enabled for large dataset - this will take significant time and storage")
