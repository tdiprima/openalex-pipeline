...
class DatasetQuery:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        top_authors = sorted(
            author_pub_counts.items(), key=itemgetter(1), reverse=True
        )[:10]
    ...
    def export_filtered_data(
        self,
        output_file: str,
        publication_filter: Optional[Callable] = None,
        author_filter: Optional[Callable] = None,
        compress: bool = True,
    ):
        """
...
if __name__ == "__main__":
    ...
    if args.list_runs:
        ...
        for run in runs:
            print(
                f"  {run['run_name']}: {run['total_authors']} authors, {run['total_publications']} publications"
            )
