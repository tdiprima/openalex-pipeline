...
class PubMedAuthorSearch:
    ...
    async def search_author(self, session: aiohttp.ClientSession, 
                          lastname: str, firstname: str) -> List[str]:
        """
        ...
        queries = [
            ...
            f"{lastname} {firstname[0]}[Author]" if firstname else f"{lastname}[Author]"
        ]
    ...
    async def fetch_article_details(self, session: aiohttp.ClientSession, 
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for article in articles:
                for author in article['authors']:
                    ...
                    lastname_match = author.get('lastname', '').lower() == lastname.lower()
                    firstname_match = False
                    
                    if firstname and author.get('firstname'):
                        # Check if first initial matches or full firstname matches
                        firstname_match = (
                            author.get('firstname', '').lower().startswith(firstname[0].lower()) or
                            author.get('firstname', '').lower() == firstname.lower()
                        )
...
def read_authors_from_csv(filename: str) -> List[tuple]:
    ...
    try:
        with open(filename, 'r', encoding='utf-8') as csvfile:
            ...
            print(f"Using columns: Lastname='{lastname_col}', Firstname='{firstname_col}'")
...
async def search_multiple_authors(authors: List[tuple], email: str, api_key: Optional[str] = None):
    """
    ...
    for i in range(0, len(authors), batch_size):
        ...
        print(f"Processing batch {batch_num}/{total_batches} ({i+1}-{min(i+batch_size, total_authors)} of {total_authors})")
        
        tasks = [searcher.find_author_affiliations(lastname, firstname) 
                for lastname, firstname in batch]
...
def save_results(results: List[Dict], output_file: str = None):
    ...
    with open(csv_file, 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['Query', 'Found', 'Num_Papers', 'Emails', 'ORCIDs', 'Most_Recent_Affiliation', 'Years'])
        ...
        for result in results:
            ...
            if result['affiliations']:
                ...
                most_recent = max(result['affiliations'], 
                                key=lambda x: max(x['years']) if x['years'] else '0')
                most_recent_aff = most_recent['text'][:200]  # Truncate long affiliations
