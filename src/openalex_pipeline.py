...
class OpenAlexPipeline:
    ...
    async def fetch_authors(self, session: aiohttp.ClientSession, max_results: int = 200):
        ...
        async with session.get(url, params=params) as resp:
            ...
            for item in data.get("results", []):
                author = Author(
                    ...
                    affiliations=[aff.get("display_name", "") for aff in item.get("affiliations", [])]
                )
                authors.append(author)
            return authors
    
    async def fetch_publications(self, session: aiohttp.ClientSession, author_id: str, max_results: int = 200):
        ...
        async with session.get(url, params=params) as resp:
            ...
            for item in data.get("results", []):
                pub = Publication(
                    ...
                    pdf_url=item.get("primary_location", {}).get("pdf_url") if item.get("primary_location") else None,
    ...
    async def save_author(self, author: Author):
        ...
        """, author.id, author.name, author.works_count, author.cited_by_count, author.affiliations)
    ...
    async def save_publication(self, pub: Publication):
        ...
        """, pub.id, pub.title, pub.doi, pub.publication_year, pub.pdf_url, pub.authors, pub.abstract)
    ...
    async def run(self, max_authors: int = 50, max_pubs_per_author: int = 100):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for i, author in enumerate(authors):
                ...
                pubs = await self.fetch_publications(session, author.id, max_pubs_per_author)
