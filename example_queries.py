...
def example_queries(run_directory: str):
    ...
    print("-" * 30)
    sb_authors = [author for author in authors.values() if author['stonybrook_mentions'] > 0]
    ...
    pdfs_downloaded = sum(1 for pub in publications if pub['processing']['pdf_downloaded'])
    texts_extracted = sum(1 for pub in publications if pub['processing']['text_extracted'])
    ...
    print(f"Summaries generated: {summaries_generated} ({summaries_generated/total_pubs*100:.1f}%)")
    
    # Query 5: Most productive authors
    print("\n6. MOST PRODUCTIVE AUTHORS (by citations)")
    print("-" * 30)
    sorted_authors = sorted(authors.values(), key=lambda x: x['cited_by_count'], reverse=True)
    for author in sorted_authors[:10]:  # Top 10
        print(f"  {author['name']}: {author['cited_by_count']} citations, {author['publications_processed']} pubs processed")
    ...
    sample_author_pubs = [pub for pub in publications if pub['author_id'] == sample_author['id']]
    ...
    for pub in sample_author_pubs:
        ...
        sb_status = f"SB: {pub['processing']['stonybrook_validation'].get('count', 0)}" if pub['processing']['stonybrook_validation'] else "SB: 0"
        print(f"  - {pub['title'][:50]}... ({pub['publication_year']}) [{status}] [{sb_status}]")
...
def jq_examples():
    ...
    examples = [
        ...
        ("Find authors with >1000 citations", "jq 'select(.cited_by_count > 1000)' authors.jsonl"),
        ("Get publications from 2023", "jq 'select(.publication_year == 2023)' publications.jsonl"),
        ("Count PDFs downloaded", "jq 'select(.processing.pdf_downloaded == true)' publications.jsonl | wc -l"),
        ("Find publications with Stony Brook mentions", "jq 'select(.processing.stonybrook_validation.found == true)' publications.jsonl"),
        ("Get publication titles only", "jq -r '.title' publications.jsonl"),
        ("Join: Get author name for each publication", "jq -r '[.author_name, .title] | @csv' publications.jsonl")
    ]
