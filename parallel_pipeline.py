...
class ParallelResearchPipeline:
    ...
    def _process_author_batch(
        self, author_batch: List[Author], num_pubs: int
    ) -> List[Tuple[Author, List[Publication], List]]:
        """
            ...
            for future in concurrent.futures.as_completed(future_to_author):
                ...
                try:
                    ...
                    if result:
                        ...
                        print(
                            f"✓ Completed: {author.name} ({len(result[1])} publications)"
                        )
    ...
    def _process_single_author(
        self, author: Author, num_pubs: int
    ) -> Optional[Tuple[Author, List[Publication], List]]:
        """
        ...
        try:
            ...
            # Get publications
            publications = thread_api.get_author_publications(
                author.id, max_results=num_pubs
            )
            ...
            if config.pdf.download_enabled:
                with concurrent.futures.ThreadPoolExecutor(
                    ...
                    future_to_pub = {
                        pdf_executor.submit(
                            self._process_single_publication,
                            pub,
                            idx + 1,
                            len(publications),
                            thread_processor,
                ...
                for idx, pub in enumerate(publications):
                    result = self._process_single_publication(
                        pub,
                        idx + 1,
                        len(publications),
                        thread_processor,
    ...
    async def run_async(
        self, num_authors: int = 100, num_pubs: int = 10, batch_size: int = 50
    ):
        """
        ...
        for batch_start in range(0, len(authors), batch_size):
            ...
            print(
                f"Batch {batch_num} complete. Total processed: {total_processed}/{len(authors)}"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--email", type=str, required=True, help="Your email for OpenAlex"
    )
    ...
    parser.add_argument(
        "--batch-size", type=int, default=50, help="Batch size for parallel processing"
    )
    parser.add_argument("--workers", type=int, help="Number of worker processes")

    args = parser.parse_args()

    pipeline = ParallelResearchPipeline(email=args.email, max_workers=args.workers)

    pipeline.run(
        num_authors=args.authors, num_pubs=args.pubs, batch_size=args.batch_size
    )
